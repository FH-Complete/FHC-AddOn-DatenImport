...
<head>
	...
	<title>LBS Datenmigration - IBA Course</title>
</head>
<body>
<h1>LBS Datenmigration - IBA Course</h1>';
...
$studiengang_kz=-570; // IBA

//Alle Einträge in der Tabelle lbs_iba_course durchlaufen
$qry_course = "SELECT * FROM sync.lbs_iba_course";
	...
	while($row_course = $db->db_fetch_object($result_course))
	{
		...
		if($result_sync = $db->db_query($qry_sync))
		{
			...
			else
			{
                ...
                if(!$lehrveranstaltung->save())
                {
                    logMessage("Fehler beim Update der Lehrveranstaltung aufgetreten"); 
                }
                else
                {
