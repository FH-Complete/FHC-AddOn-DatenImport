...
<head>
	...
	<title>LBS Datenmigration - IML Course</title>
</head>
<body>
<h1>LBS Datenmigration - IML Course</h1>';
...
alvs					tbl_lehreinheitmitarbeiter.semesterstunden
changed					-
changed_by				tbl_lehrveranstaltung.updatevon
courseDescription		? tbl_lvinfo
courseNumber			tbl_lehrveranstaltung.kurzbz
courseTitle				tbl_lehrveranstaltung.bezeichnung
courseWeekHours			tbl_lehrveranstaltung.semesterstunden * wochen
created					-
created_by				-
creditPoints			tbl_lehrveranstaltung.ects
dateCreation			tbl_lehrveranstaltung.insertamum
finalExam_date			tbl_lehreinheit.anmerkung
finalExam_room			tbl_lehreinheit.anmerkung
lv_typ					tbl_lehrveranstaltung.lehrform_kurzbz
modul					?
notes					tbl_lehreiheit.anmerkung
zk_F_lectorID_t			tbl_lehreinheitmitarbeiter.mitarbeiter_uid
zk_F_semesterID_t		tbl_lehrveranstaltung.semester (ueber array aufgelöst)
zk_F_yearSeason_t		-
zk_P_courseID_t			PrimaryKey
*/
...
$studiengang_kz=-573; // IML

//Alle Einträge in der Tabelle lbs_iba_course durchlaufen
$qry_course = "SELECT * FROM sync.lbs_iml_course";
	...
	while($row_course = $db->db_fetch_object($result_course))
	{
		...
		if($result_sync = $db->db_query($qry_sync))
		{
			...
			else
			{
				...
				if(!$lehrveranstaltung->save())
				{
                    logMessage("Fehler bei Update der Lehrveranstaltung aufgetreten. id:".$lehrveranstaltung_id);
                }
                else
                {
			...
			if($lehrveranstaltung_id!='')
			{
				...
				if($result_lehrfach=$db->db_query($qry_lehrfach))
				{
					...
					else
					{
						...
						{
							logMessage("Fehler bbiem Anlegen des Lehrfachs:".$lehrfach->errormsg);
